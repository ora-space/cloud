...
- **Boundary assembly**: Selects the Huawei IDaaS or GitHub adapter from `login.provider`, then builds `gateway.Store`, `gateway.Login`, `gateway.Issuer`, the rate limiter, and `gateway.NewHandler` before listening on the configured port. With `web.dist_dir` set it first opens the built frontend with `gateway.OpenWeb` (a missing `index.html` fails startup) and closes it on shutdown.
