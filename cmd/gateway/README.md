...
- **边界装配**：按 `login.provider` 构造华为 IDaaS 或 GitHub 适配器，再装配 `gateway.Store`、`gateway.Login`、`gateway.Issuer`、限流器与 `gateway.NewHandler`，监听配置端口。配置了 `web.dist_dir` 时先以 `gateway.OpenWeb` 打开前端构建目录（缺少 `index.html` 则启动失败），停机时关闭。
